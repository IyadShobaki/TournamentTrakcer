...
{
    //Note 3
    //change the following class to public and add (: IDataConnection) to implement
    ...
    public class SqlConnector : IDataConnection
    {
        ...
        {
            List<TeamModel> output; //we don't need to initialize it
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(
                GlobalConfig.CnnString(db)))
            {
        ...
        {
            List<TournamnetModel> output; 
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(
                GlobalConfig.CnnString(db)))
            {
                output = connection.Query<TournamnetModel>("dbo.spTournaments_GetAll").ToList();
                var p = new DynamicParameters();

                foreach (TournamnetModel t in output)
                {
                    // Populate Prizes 
                    p = new DynamicParameters();
